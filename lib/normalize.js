...
function normalize () {
  const PARA_WORD_LIMIT = 40 // Fine tune according to style or literature.
  const wordcount = require('wordcount')
  const cheerio = require('cheerio')
  const $ = cheerio.load(scroll)

  let normalizedHTML = scroll
  //  let newElem = ''
  ...
  // $('body').children().each(function (i, elem) {
        ...
        //     newElem = $(this);
        // }
  //   normalizedHTML = scroll
  //   // normalizedHTML + newElem
  //   newElem = ''
  // })

  outPutNormalizedBook(normalizedHTML)

  function splitPara (para, ratio) {
